...
void Truck::main() {
    ...
    for (;;) {
        ...
        // get new shipment
        try {
            _Enable { plant.getShipment(cargo); }
            printer.print(Printer::Kind::Truck, Truck::States::PickedUp,
                          shipmentCount());
...
void Truck::fillStock(VendingMachine *machine) {
    printer.print(Printer::Kind::Truck, Truck::States::BeginDelivery,
                  machine->getId(), shipmentCount());
    int noReplen = 0;
    ...
    // failed filling vend
    if (noReplen > 0)
        printer.print(Printer::Kind::Truck, Truck::States::FailedFillingVM,
                      machine->getId(), noReplen);
    printer.print(Printer::Kind::Truck, Truck::States::DoneDelivery,
                  machine->getId(), shipmentCount());
    machine->restocked();
